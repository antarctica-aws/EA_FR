...
poly <- readOGR('AOI/HASHI/hashi_west.geojson')
#spoly$year, interv = spoly$interventi, interv2 = spoly$manageType
poly$pname <- paste('poly',poly$FID,sep="_")
poly$year <- 1986
poly$interventi <- 'exclosure'
poly$manageType <- 'NR'
...
ras_temp <-  raster(paste('agbd_predictions/shinyanga1_ts_out/agbd_2021_xgboost_sqrt_nbr_lhs_33_', pattern,'_ls_lhs_medoid_',tile,'_2021_NBR_3by3_dur8_biome1_val_v2-0000002560-0000000000.tif', sep=''))
...
for (pred_year in 2021:1986){
  ...
  annual_ras1 <- raster(paste('agbd_predictions/shinyanga1_ts_out/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_', pattern,'_ls_lhs_medoid_',tile,'_', pred_year,'_NBR_3by3_dur8_biome1_val_v2-0000002560-0000005120.tif', sep=''))
  annual_ras2 <- raster(paste('agbd_predictions/shinyanga1_ts_out/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_', pattern,'_ls_lhs_medoid_',tile,'_', pred_year,'_NBR_3by3_dur8_biome1_val_v2-0000000000-0000005120.tif', sep=''))
  ...
  for (pl in 1:length(poly)){
    ...
    if (!class(try(intersect(annual_ras1,spoly),T ))=='try-error'){
      ...
      annual1_sub2 <- annual1_sub_covar %>% as.data.frame(., xy = TRUE) %>%
        mutate(manage_stratum = spoly$year, interv = spoly$interventi, interv2 = spoly$manageType, 
               polyID= spoly$pname,estimat_year =pred_year, ras_sub =sub)
    ...
    if (!class(try(intersect(annual_ras2,spoly),T ))=='try-error'){
      ...
      annual1_sub2_v2 <- annual1_sub_covar_v2 %>% as.data.frame(., xy = TRUE) %>%
        mutate(manage_stratum = spoly$year, interv = spoly$interventi, interv2 = spoly$manageType, 
               polyID= spoly$pname,estimat_year =pred_year, ras_sub =sub2)
    ...
    if (!class(try(intersect(annual_ras3,spoly),T ))=='try-error'){
      ...
      annual1_sub2_v3 <- annual1_sub_covar_v3 %>% as.data.frame(., xy = TRUE) %>%
        mutate(manage_stratum = spoly$year, interv = spoly$interventi,  interv2 = spoly$manageType,polyID= spoly$pname,
               estimat_year =pred_year, ras_sub =sub3)
...
write.csv( tile_agbd_by_manage, paste("agbd_predictions/shinyanga1_ts_out/agbd_collrm_by_planting_px",
