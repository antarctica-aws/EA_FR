...
poly <- readOGR('AOI/TISTKEN/TISTKEN_tsgss_t168060_2010on.geojson')
# poly <- poly[3001:3536,]
...
ras_temp <-  raster(paste('agbd_predictions/TISTKEN_ts_out/agbd_2021_xgboost_sqrt_nbr_lhs_33_', pattern,'_ls_lhs_medoid_',tile,'_2021_NBR_3by3_dur8_biome1_val_v2-0000002560-0000005120.tif', sep=''))
...
tile_agbd_by_manage <- foreach(pred_year=2021:1986, .combine = rbind, .packages=c('sp','magrittr', 'dplyr','tidyr','raster')) %dopar% {   
  ...
  annual_ras1 <- raster(paste('agbd_predictions/TISTKEN_ts_out/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_', pattern,'_ls_lhs_medoid_',tile,'_', pred_year,'_NBR_3by3_dur8_biome1_val_v2-0000002560-0000000000.tif', sep=''))
  annual_ras2 <- raster(paste('agbd_predictions/TISTKEN_ts_out/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_', pattern,'_ls_lhs_medoid_',tile,'_', pred_year,'_NBR_3by3_dur8_biome1_val_v2-0000002560-0000005120.tif', sep=''))
  annual_ras3 <- raster(paste('agbd_predictions/TISTKEN_ts_out/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_', pattern,'_ls_lhs_medoid_',tile,'_', pred_year,'_NBR_3by3_dur8_biome1_val_v2-0000002560-0000002560.tif', sep=''))
  annual_ras4 <- raster(paste('agbd_predictions/TISTKEN_ts_out/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_', pattern,'_ls_lhs_medoid_',tile,'_', pred_year,'_NBR_3by3_dur8_biome1_val_v2-0000005120-0000000000.tif', sep=''))
  annual_ras5 <- raster(paste('agbd_predictions/TISTKEN_ts_out/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_', pattern,'_ls_lhs_medoid_',tile,'_', pred_year,'_NBR_3by3_dur8_biome1_val_v2-0000005120-0000002560.tif', sep=''))
  annual_ras6 <- raster(paste('agbd_predictions/TISTKEN_ts_out/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_', pattern,'_ls_lhs_medoid_',tile,'_', pred_year,'_NBR_3by3_dur8_biome1_val_v2-0000005120-0000005120.tif', sep=''))
  ...
  annual_agbd_by_manage <- foreach(pl=1:length(poly), .combine = rbind, .packages=c('sp','magrittr', 'dplyr','tidyr','raster')) %dopar% {
    ...
    if (!class(try(raster::intersect(annual_ras5,spoly),T ))=='try-error'){
      ...
      annual1_sub2_v5 <- annual1_sub_covar_v5 %>% as.data.frame(., xy = TRUE) %>%
        mutate(manage_stratum = spoly$year, interv = spoly$interventi,  interv2 = spoly$manageType,estimat_year =pred_year, ras_sub =sub5)
    ...
    if (!class(try(raster::intersect(annual_ras6,spoly),T ))=='try-error'){
      ...
      annual1_sub2_v6 <- annual1_sub_covar_v6 %>% as.data.frame(., xy = TRUE) %>%
        mutate(manage_stratum = spoly$year, interv = spoly$interventi,  interv2 = spoly$manageType,estimat_year =pred_year, ras_sub =sub6)
...
write.csv( tile_agbd_by_manage, paste("agbd_predictions/TISTKEN_ts_out/agbd_collrm_by_planting_px",
                                      pattern,"_tsgss_AR_2020onAll_covar.csv", sep=""))
