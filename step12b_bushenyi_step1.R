...
library(raster)
library(rgdal)
# poly <- readOGR('AOI/ecotrust\bushenyi_tsgss.geojson.geojson')
# poly <- readOGR('AOI/ecotrust/bushenyi_tsgss_ANR.geojson')
poly <- readOGR('AOI/ecotrust/ecotrust_tsmbf_wYear.geojson')
poly$PlantingYe <- poly$year
...
start.time <- Sys.time()
tile_agbd_by_manage <- data.frame()   #**for bushenyi tsgss the subsets are 0000005120-0000002560 and 0000002560-0000005120***
for (pred_year in 2021:1986){   #1986
  print(pred_year)
  # pred_year <- as.character(pred_year)
  if (pred_year < 2019  && pred_year>2015){
    annual_ras1 <- raster(paste('agbd_predictions/kibale_ts_out3/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_tile+', pattern,'_ls_lhs_medoid_173060_', pred_year,'_NBR_3by3_dur8_biome1_val-0000002560-0000005120.tif', sep=''))
    annual_ras2 <- raster(paste('agbd_predictions/kibale_ts_out3/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_tile+', pattern,'_ls_lhs_medoid_173060_', pred_year,'_NBR_3by3_dur8_biome1_val-0000000000-0000005120.tif', sep=''))
    
  } else if (pred_year<=2015){
    annual_ras1 <- raster(paste('agbd_predictions/kibale_ts_out3/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_tile+', pattern,'_ls_lhs_medoid_173060_', pred_year,'_NBR_3by3_dur8_biome1_val_v2-0000002560-0000005120.tif', sep=''))
    annual_ras2 <- raster(paste('agbd_predictions/kibale_ts_out3/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_tile+', pattern,'_ls_lhs_medoid_173060_', pred_year,'_NBR_3by3_dur8_biome1_val_v2-0000000000-0000005120.tif', sep=''))
    
    
  } else if (pred_year %in% c(2021,2020,2019)){
    annual_ras1 <- raster(paste('agbd_predictions/kibale_ts_out3/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_tile+', pattern,'_ls_lhs_medoid_173060_', pred_year,'_NBR_3by3_dur8_biome1-0000002560-0000005120.tif', sep=''))
    annual_ras2 <- raster(paste('agbd_predictions/kibale_ts_out3/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_tile+', pattern,'_ls_lhs_medoid_173060_', pred_year,'_NBR_3by3_dur8_biome1-0000000000-0000005120.tif', sep=''))
  }
