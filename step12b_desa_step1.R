...
poly <- readOGR('AOI/Desa/desa_AR_ANR_NR.shp')#desa_interv_original.shp') #wf_new_interv4_chfor.shp')

pattern <- 'desa2+1sig_sen98_sol_v4_rfr7_thre_tuned_dcol'

ras_temp <- raster(paste('agbd_predictions/desa_ts_out/rfr7_ts/agbd_2021_xgboost_sqrt_nbr_lhs_33_', pattern,'_ls_lhs_medoid_168051_2021_NBR_3by3_dur8_biome1_val_v2-0000000000-0000000000.tif', sep=''))
...
for (pred_year in 2021:1986){
  ...
  annual_ras1 <- raster(paste('agbd_predictions/desa_ts_out/rfr7_ts/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_', pattern,'_ls_lhs_medoid_168051_', pred_year,'_NBR_3by3_dur8_biome1_val_v2-0000000000-0000000000.tif', sep=''))
  annual_ras2 <- raster(paste('agbd_predictions/desa_ts_out/rfr7_ts/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_', pattern,'_ls_lhs_medoid_168050_', pred_year,'_NBR_3by3_dur8_biome1_val_v2-0000005120-0000000000.tif', sep=''))
  
  annual_ras3 <- raster(paste('agbd_predictions/desa_ts_out/rfr7_ts/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_', pattern,'_ls_lhs_medoid_168051_', pred_year,'_NBR_3by3_dur8_biome1_val_v2-0000000000-0000002560.tif', sep=''))
  annual_ras4 <- raster(paste('agbd_predictions/desa_ts_out/rfr7_ts/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_', pattern,'_ls_lhs_medoid_168050_', pred_year,'_NBR_3by3_dur8_biome1_val_v2-0000002560-0000000000.tif', sep=''))
  
  annual_ras5 <- raster(paste('agbd_predictions/desa_ts_out/rfr7_ts/agbd_', pred_year,'_xgboost_sqrt_nbr_lhs_33_', pattern,'_ls_lhs_medoid_168051_', pred_year,'_NBR_3by3_dur8_biome1_val_v2-0000002560-0000000000.tif', sep=''))
  ...
  for (pl in 1:length(poly)){
    ...
    sub4 <- '4'
    if (!class(try(intersect(annual_ras4,spoly),T ))=='try-error'){
      # print('4')
      # annual1_sub_v4 <- raster::crop(annual_ras4, spoly)
      # annual1_sub2_v4 <- raster::mask(annual1_sub_v4, spoly) %>% as.data.frame(., xy = TRUE) %>%
    ...
    sub5 <- '5'
    if (!class(try(intersect(annual_ras5,spoly),T ))=='try-error'){
      # print('5')
      ...
      # annual1_sub_v5 <- raster::crop(annual_ras5, spoly)
      # annual1_sub2_v5 <- raster::mask(annual1_sub_v5, spoly) %>% as.data.frame(., xy = TRUE) %>%
      #   mutate(manage_stratum = spoly$year, interv = spoly$interventi,  interv2 = spoly$manageType, estimat_year =pred_year, ras_sub =sub5)
      # annual1_sub2_v5 <-  annual1_sub2_v5 %>% dplyr::filter(!is.na(est_agbd))
    } else{
...
write.csv( tile_agbd_by_manage, paste("agbd_predictions/desa_ts_out/agbd_collrm_by_planting_px",
