#this script is for estimating ANR project analysis in MGS biome
...
tile_agbd_by_manage_all <- stratified(tile_agbd_by_manage_t, "manage_stratum",0.5)  #for analaysis samp=0.5, for plotting 0.999
...
#------------------this is for the ANR 2020 gourp--------------------
subset1 <- tile_agbd_by_manage3 %>%
  ...
  dplyr::filter(manage_group %in% c("ANR_Plant year 2020")) %>% #"ANR_Plant year 2006", 
...
agbd_rfmt3 <- agbd_rfmt30 %>% dplyr::filter(siteLength -siteNAlen>=8) #the difference represents at least n needs to have value
...
library(did)
# agbd_rfmt3 <- agbd_rfmt3 %>% dplyr::filter(year>1993)  #for plotting clearity
...
# 
save(mw.attgt,file = 'ATT_results_save/MGS_ANR_2020_group_attObj_v2.Rdata')



#########----------------------below is for 2006 group---------------------#############
...
save(mw.attgt2,file = 'ATT_results_save/MGS_ANR_2006_group_attObj_v2.Rdata')
