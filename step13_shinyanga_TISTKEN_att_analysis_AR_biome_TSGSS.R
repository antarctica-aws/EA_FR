...
tile_agbd_by_manage_tisttza <- read.csv('agbd_predictions/TISTKEN_ts_out/tile_agbd_by_manage_tisttza.csv') %>% 
  dplyr::select(-c('X.1'))

#combine different projects
tile_agbd_by_manage <- rbind(tile_agbd_by_manage_shinwest,  tile_agbd_by_manage_tistken)
...
tile_agbd_by_manage_all <- stratified(tile_agbd_by_manage_t, "manage_stratum",0.6)  #for analysis samp = 0.6


######calcualte the mean and median AGBD by managemnt year and plot the time series of mean and median change
...
agbd_rfmt3 <- agbd_rfmt30 %>% dplyr::filter(siteLength-siteNAlen>=2)#the difference represents at least n needs to have value
...
summary(agg.simple)
# ggsave(paste("Fig/TSGSS_AR_event_study_2012-2019.png", sep=""), plot= ggdid(mw.attgt),
#        width =4800, height =2250, units = "px")
...
# save(mw.attgt, file = 'ATT_results_save/TSGSS_AR_2012~2019_group_attObj.Rdata')
