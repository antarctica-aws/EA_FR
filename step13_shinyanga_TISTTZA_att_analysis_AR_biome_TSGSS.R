...
pattern <- 'TISTKEN+2sig_sen96_sol_beam_nbr'
tile <- '168060'
tile_agbd_by_manage_tistken <- read.csv(paste("agbd_predictions/TISTKEN_ts_out/agbd_collrm_by_planting_px",
                                              pattern,"_tsgss_AR_2020on4_covar.csv", sep="")) %>%
  dplyr::mutate(interv='TISTKEN',interv2='AR') %>%#dplyr::filter(manage_stratum %in%c(2013, 2014)) %>% 
...
tile <- '168064'
tile_agbd_by_manage_tisttza <- read.csv(paste("agbd_predictions/dodoma1_ts_out/agbd_collrm_by_planting_px",
               pattern,"_tsgss_TISTTZA_ANR_t168064_covar.csv", sep="")) %>% 
  dplyr::mutate(interv='TISTTZA',interv2='AR') %>%#dplyr::filter(manage_stratum %in%c(2013, 2014)) %>% 
  dplyr::select(X,x,y, est_agbd, d2roads, pop_cnt_2000, slope,  dem, wc_prec_1990.1999, wc_tavg_1990.1999, soilType,
                manage_stratum, interv, interv2, estimat_year, ras_sub)
...
tile_agbd_by_manage_shinwest <- read.csv( paste("agbd_predictions/shinyanga1_ts_out/agbd_collrm_by_planting_px",
  ...
  # dplyr::filter(polyID %in% c('poly_1','poly_2', 'poly_3')) %>% 
  dplyr::select(X,x,y, est_agbd, d2roads, pop_cnt_2000, slope,  dem, wc_prec_1990.1999, wc_tavg_1990.1999, soilType,
                manage_stratum, interv, interv2, estimat_year, ras_sub)

pattern <- 'mungere+2sig_senCov_sol_beam_nbr'  #unless for treated after 2012 or before 1991, not useful
# tile_agbd_by_manage_mungere <- read.csv( paste("agbd_predictions/mungere_ts_out/agbd_collrm_by_planting_px",pattern,"_tsgss_NR_covar.csv", sep="")) %>%
#   dplyr::mutate(interv='mungere',interv2='NR') %>%
#   dplyr::select(X,x,y, est_agbd, d2roads, pop_cnt_2000, slope,  dem, wc_prec_1990.1999, wc_tavg_1990.1999, soilType,
#                 manage_stratum, interv, interv2, estimat_year, ras_sub)
# 
tile_agbd_by_manage_chyulu <- read.csv( paste("agbd_predictions/mungere_ts_out/agbd_collrm_by_planting_px",pattern,"_tsgss_chyulu_hills_AR_covar.csv", sep="")) %>%
  dplyr::mutate(interv='Chyulu_hills',interv2='AR') %>%
  dplyr::select(X,x,y, est_agbd, d2roads, pop_cnt_2000, slope,  dem, wc_prec_1990.1999, wc_tavg_1990.1999, soilType,
                manage_stratum, interv, interv2, estimat_year, ras_sub)

tile_agbd_by_manage_chyulu$loc <- paste(tile_agbd_by_manage_chyulu$x, tile_agbd_by_manage_chyulu$y, sep="_")
sloc <- sample(unique(tile_agbd_by_manage_chyulu$loc),length(unique(tile_agbd_by_manage_chyulu$loc))*0.01)
tile_agbd_by_manage_chyulu <-  tile_agbd_by_manage_chyulu[tile_agbd_by_manage_chyulu$loc %in% sloc,]
tile_agbd_by_manage_chyulu <- tile_agbd_by_manage_chyulu[,!(names(tile_agbd_by_manage_chyulu) %in% c('loc'))]


# tile_agbd_by_manage_amboseli <- read.csv( paste("agbd_predictions/mungere_ts_out/agbd_collrm_by_planting_px",pattern,"_tsgss_Amboseli_NR_covar.csv", sep="")) %>%
#   dplyr::mutate(interv='Amboseli',interv2='NR') %>%
#   dplyr::select(X,x,y, est_agbd, d2roads, pop_cnt_2000, slope,  dem, wc_prec_1990.1999, wc_tavg_1990.1999, soilType,
#                 manage_stratum, interv, interv2, estimat_year, ras_sub)

#combine different projects
tile_agbd_by_manage <- rbind(tile_agbd_by_manage_shinwest,  tile_agbd_by_manage_tisttza)
...
tile_agbd_by_manage_all <- stratified(tile_agbd_by_manage_t, "manage_stratum",0.6)  # analysis samp=0.6 
...
tile_agbd_by_manage3 <- tile_agbd_by_manage_all %>%   #regroup
  ...
  # dplyr::mutate(manage_group= ifelse(manage_group %in% c("ANR_Plant year 2004","ANR_Plant year 2006"),"ANR_Plant year 2006",manage_group)) %>% 
  # dplyr::mutate(manage_group= ifelse(manage_group %in% c("ANR_Plant year 2008", "ANR_Plant year 2009"),"ANR_Plant year 2009",manage_group)) %>% 
...
subset1 <- tile_agbd_by_manage3 %>%
  dplyr::filter(manage_group %notin% c("NR_Plant year 1986")) %>%
  # dplyr::filter(manage_group %in% c("AR_Plant year 2013")) %>%
  # dplyr::filter(manage_group %in% c("ANR_Plant year 2006", "ANR_Plant year 2009","ANR_Plant year 2012")) %>%
...
# # agbd_rfmt3$log_temp <-log(agbd_rfmt3$wc_tavg_1990.1999)
# agbd_rfmt3[(agbd_rfmt3$first.treat==0)& (agbd_rfmt3$year %in% c(2020:2021)),]$cma  <-  NA
# agbd_rfmt3[(agbd_rfmt3$first.treat==0)& (agbd_rfmt3$year %in% c(2020:2021)),]$cma2  <-  NA
# agbd_rfmt3[(agbd_rfmt3$first.treat==0),]$treat  <-  0

library(viridis)
rateFig <- ggplot(data=agbd_rfmt3, aes(x=year, y=cma2, group=loc, color=loc)) +  #good for viz the overall trends in each group, also group homogenity 
  facet_wrap(vars(first.treat))+
  geom_line( size=0.7, alpha=0.6)+
  scale_color_viridis(discrete = F) +
  theme(
    legend.position="none",
    plot.title = element_text(size=14)) +
  ggtitle("Pixel-level AGBD trajectories by management groups") +
  theme_bw()

rateFig


lineFig <- ggplot(data=agbd_rfmt3, aes(x=year, y=est_agbd, group=loc)) +
  facet_wrap(vars(first.treat))+
  geom_line(color='grey',linetype="dashed", size=0.6, alpha=0.5)+
  geom_point(aes(color=first.treat), size=1)+
  # scale_color_viridis(discrete = F)+
  ylab('AGBD (Mg/ha)')+xlab('Year of Predictions')+
  theme(
    plot.title = element_text(size=14)) +
  ggtitle("Pixel-level AGBD trajectories by management groups") +
...
ggsave(paste("Fig/kibale_AR+control_agbd_ts_", pattern, "_by_managment.png", sep=""), plot= lineFig, width =2800, height = 1600, units = "px")


##############try staggered ATT method ---------------------
library(did)
## n>=8 control by slode and sit  0.082         0.852    -1.5879      1.7519 
## n > =5 , by log_precp and slpe 3.3175        4.5633    -5.6265     12.2614
#samp 0.6 n>=3, by log_precp and d2roads, 4.0401        2.3643    -0.5938      8.6741 
#         n>=4, 5.0423        2.6395    -0.1311     10.2156 
# estimate group-time average treatment effects without covariates
mw.attgt <- att_gt(yname = "cma2",
                   gname = "first.treat",
                   idname = "loc",
                   tname = "year",
                   xformla = ~log_precp+d2roads,
...
summary(agg.simple)
ggsave(paste("Fig/TSGSS_AR_event_study_2000.png", sep=""), plot= ggdid(mw.attgt),
...
save(mw.attgt, file = 'ATT_results_save/TSGSS_AR_2000_group_attObj.Rdata')
